...
package mapnik2 {
  ...
  trait TextProperties {
    ...
    private def extractLabel(atts: Map[String, String], text: Option[String]) =
      <Label>
        { for {
    ...
    def convertTextSymbolizer(text: Elem): Node = {
      ...
      <TextSymbolizer>
        { extractLabel(attmap, Some(text.text).filter(_ nonEmpty)) }
        { extractFont(attmap) }
    ...
    def convertShieldSymbolizer(shield: Elem): Seq[Node] = {
      ...
      <TextSymbolizer>
        { extractLabel(attmap, Some(shield.text).filter(_ nonEmpty)) }
        { extractFont(attmap) }
        <LabelPlacement>
...
package mapnik1 {
  ...
  trait TextProperties {
    ...
    def extractVendorParams(atts: Map[String, String]) = {
      Seq(
        atts.get("minimum_distance").map(x => <VendorOption name="spaceAround">{x}</VendorOption>),
        atts.get("spacing").map(x => <VendorOption name="minGroupDistance">{x}</VendorOption>),
        atts.get("wrap_width").map(x => <VendorOption name="autoWrap">{x.toInt * 5}</VendorOption>)
      ).flatten
    }
